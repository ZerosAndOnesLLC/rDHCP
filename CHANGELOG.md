...
### Changed
- Updated `base64` to 0.23.
- Refreshed `Cargo.lock` to the latest compatible dependency releases.

## [0.15.1] - 2026-07-25

### Changed
- Refreshed `Cargo.lock` to the latest compatible dependency releases.
- The crates.io package no longer bundles internal development files (`plan.md`, `docs/`, `bench/`, `.github/`).
- Benchmark harness now samples RSS over time as a memory-leak regression guard (development-only; not part of the shipped binary).
